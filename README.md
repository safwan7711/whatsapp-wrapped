...
The `data.json` file is the shared contract between the analyzer (`analyze.py`) and the frontend (`app.js`). It is structured into three main parts:

### 1. Group-Level Statistics (`group`)

This contains statistics computed for the entire chat.

```json
"group": {
  "busiest_days": [[date, count], ...],
  "longest_silence": number,
  "night_owls": [[name, count], ...],
  "ghosts": [[name, replies_received], ...],
  "hype_persons": [[name, avg_response_time], ...],
  "conversation_starters": [[name, score], ...],
  "person_achievements": {
    "name": [[stat_name, rank], ...]
  }
}
...
### 2. Stats-vs-Time (`date_message_count`)

```json
"date_message_count": [[date, count], ...]
```

Stores number of messages per day. Visualized this using a plot and tracking no. of messages each day

---

### 3. Per-Person Statistics (`persons`)

This is a dictionary keyed by user name:
...
"persons": {
  "Name": {
    ...
    "total_words": number,
    "most_used_emojis": [emoji1, emoji2, ...],
    "avg_response_time": number,
    "activity_concentration": number,
    "person_achievements": [[stat_name, rank], ...]
  }
}
```
...
This schema is clearly separate:

- **Group-level insights** (`group`)
- **Time-based trends** (`date_message_count`)
- **Individual user statistics** (`persons`)
